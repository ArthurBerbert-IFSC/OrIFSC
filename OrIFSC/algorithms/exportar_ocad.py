...
# zoom máximo do Google Satellite (lyrs=s)
ZOOM_MAX = 20
# teto por lado do mosaico (limite técnico: memória + OCAD 10 32-bit)
MAX_PX = 16384
...
class ExportarOCAD(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ...
        # Mostra a escala já definida no projeto (lida da folha), para o usuário
        # confirmar antes de exportar — ela não aparece como campo no diálogo.
    ...
    def _exportar_satelite(self, extent, crs, offset_zoom, pasta, feedback):
        ...
        # 2. Melhor zoom que cabe no teto de pixels, menos o offset de
        # qualidade.
        ...
        # 4. Georreferencia em 3857 e reprojeta para o CRS da folha (alinha às
        # curvas).
        caminho = os.path.join(pasta, 'satelite_orifsc.tif')
        self._georref_e_reprojeta(img, bounds3857, crs, caminho, feedback)
        return caminho
    ...
    def _contar_tiles(self, rect, zoom):
    ...
    def _baixar_mosaico(self, rect, zoom, feedback):
        ...
        px_min = (rect.xMinimum() + ORIGIN_SHIFT) / res
        px_max = (rect.xMaximum() + ORIGIN_SHIFT) / res
        py_min = (ORIGIN_SHIFT - rect.yMaximum()) / \
            res     # topo (y invertido)
        ...
        # Progresso visível: barra (0-50%) + uma linha a cada ~5% das tiles e um
        # "heartbeat" enquanto espera respostas (ver rede.baixar_varios).
        passo_log = max(1, total_tiles // 20)

        def _prog(feito, tot):
            feedback.setProgress(int(50 * feito / tot))
            if feito == tot or feito % passo_log == 0:
                feedback.pushInfo(f'  tiles: {feito}/{tot}')

        def _heartbeat(feito, tot, andamento):
            feedback.pushInfo(
                f'  ...baixando ({feito}/{tot}, {andamento} em andamento)')
        ...
        # Re-tentativas mantêm a imagem completa mesmo sob throttling do
        # Google.
        dados_por_url = baixar_varios(
            url_de.values(), user_agent=UA, max_conc=12, timeout_ms=20000,
            tentativas=3, cancelado=feedback.isCanceled,
            progresso=_prog, heartbeat=_heartbeat)

        feedback.pushInfo('Montando o mosaico das tiles...')
        falhas = 0
        passo_m = max(1, total_tiles // 10)
        for i, (tx, ty) in enumerate(tiles, 1):
            ...
            if dados:
                ...
                if tile.loadFromData(dados):
                    pintor.drawImage(
                        (tx - tx0) * TILE, (ty - ty0) * TILE, tile)
                    ok = True
            if not ok:
                falhas += 1
            if i % passo_m == 0 or i == total_tiles:
                feedback.setProgress(
                    50 + int(5 * i / total_tiles))   # 50 -> 55
                feedback.pushInfo(f'  montando: {i}/{total_tiles}')
        pintor.end()
        if falhas == len(tiles):
            raise QgsProcessingException(
        ...
        esq, topo = int(round(px_min - tx0 * TILE)
                        ), int(round(py_min - ty0 * TILE))
    ...
    def _georref_e_reprojeta(self, img, bounds3857,
                             crs, destino_tif, feedback):
        ...
        feedback.pushInfo('Salvando a imagem base do mosaico '
                          '(pode levar alguns segundos)...')
        if not img.save(tmp_png, 'PNG'):
            raise QgsProcessingException(
                'Não foi possível gerar a imagem temporária.')
        try:
            ulx, uly, lrx, lry = bounds3857
            feedback.pushInfo('Georreferenciando a imagem (EPSG:3857)...')
            ds_3857 = gdal.Translate(tmp_3857, tmp_png, outputSRS='EPSG:3857',
                                     outputBounds=[ulx, uly, lrx, lry],
                                     callback=self._cb_gdal(feedback, 'preparando', 55, 57))
            if ds_3857 is not None:
                ds_3857.FlushCache()
            ...
            ds_out = gdal.Warp(destino_tif, tmp_3857, dstSRS=crs.authid(),
                               ...
                               callback=self._cb_gdal(feedback, 'reprojetando', 57, 60))
            if ds_out is None:
            ...
            ds_out.FlushCache()
            # fecha/libera o GeoTIFF final (sem handle preso)
            ds_out = None
        finally:
    ...
    @staticmethod
    def _cb_gdal(feedback, rotulo, p0, p1):
        ...
        def _cb(complete, message, _dados):
            if feedback.isCanceled():
                return 0
